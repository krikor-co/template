...
/**
 * Register-step guard, in the Suspense guard shell shape (docs/guards.md →
...
export default function RegisterLayout({ children }: { children: React.ReactNode }) {
  return (
    <Suspense fallback={<RegisterGuardFallback />}>
      <RegisterGuard>{children}</RegisterGuard>
    </Suspense>
  )
}

async function RegisterGuard({ children }: { children: React.ReactNode }) {
 ...
 */
function RegisterGuardFallback() {
  return (
