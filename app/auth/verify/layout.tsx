...
/**
 * Verify-step guard, in the Suspense guard shell shape (docs/guards.md →
...
export default function VerifyLayout({ children }: { children: React.ReactNode }) {
  return (
    <Suspense fallback={<VerifyGuardFallback />}>
      <VerifyGuard>{children}</VerifyGuard>
    </Suspense>
  )
}

async function VerifyGuard({ children }: { children: React.ReactNode }) {
