...
export async function sendLoginOtp(
  formData: FormData
): Promise<{ success: false; error: string } | { success: true, redirectTo: string } | never> {
  ...
  if (existing.length === 0) {
    ...
    cookieStore.set(AUTH_TRANSITION_COOKIES.identify, '1',   transitionOpts)
    return { success: true, redirectTo: route.exits.register({ returnTo }) }
  }
